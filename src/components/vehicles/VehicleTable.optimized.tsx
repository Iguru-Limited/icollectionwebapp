...
const VehicleRow = memo(({ vehicle, index, onReceiptClick, onTransactionClick }: {
  vehicle: Vehicle;
  index: number;
  onReceiptClick: (vehicleId: number) => void;
  onTransactionClick: (vehicleId: number) => void;
}) => {
  return (
      ...
      <TableCell className="text-right">
        <div className="flex justify-end gap-1 md:gap-2">
          <Button
            ...
            onClick={() => onReceiptClick(vehicle.vehicle_id)}
          >
          ...
          <Button
            ...
            onClick={() => onTransactionClick(vehicle.vehicle_id)}
          >
...
export function VehicleTable({ vehicles, isLoading = false }: VehicleTableProps) {
  ...
  const handleReceiptClick = useCallback((vehicleId: number) => {
    setSelectedVehicleId(vehicleId);
    router.push('/user/collection');
  }, [router, setSelectedVehicleId]);

  const handleTransactionClick = useCallback((vehicleId: number) => {
    router.push(`/user/report/${vehicleId}`);
  }, [router]);
