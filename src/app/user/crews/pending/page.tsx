...
export default function CrewPendingListPage() {
  ...
  const filteredCrews = useMemo(() => {
    if (!searchQuery.trim()) return unassignedCrews;
    const q = searchQuery.toLowerCase();
    return unassignedCrews.filter(c =>
  ...
  const activeCount = unassignedCrews.filter(c => c.active === '1').length;
  const inactiveCount = unassignedCrews.filter(c => c.active !== '1').length;
    ...
    <PageContainer>
      ...
      <main className="px-4 pb-24 max-w-md mx-auto">
        ...
        <div className="bg-purple-50 rounded-2xl p-4 mb-4 flex items-start gap-3">
          <div className="bg-red-800 rounded-full p-2.5 mt-1">
            <svg className="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </div>
          <div className="flex-1">
            <h2 className="text-lg font-semibold text-gray-900">Pending Crew</h2>
                ...
                <Card key={crew.crew_id} className="p-4 rounded-2xl bg-white shadow-sm">
                  <div className="flex items-start gap-3">
                    ...
                    <Link href={`/user/crews/${crew.crew_id}`}>
                      <Button className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-xs font-medium">
