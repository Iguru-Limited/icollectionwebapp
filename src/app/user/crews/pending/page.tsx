...
export default function CrewPendingCategoriesPage() {
  ...
  const driversCount = useMemo(() => unassignedCrews.filter(c => c.crew_role_id === '3' || c.role_name?.toUpperCase() === 'DRIVER').length, [unassignedCrews]);
  const conductorsCount = useMemo(() => unassignedCrews.filter(c => c.crew_role_id === '12' || c.role_name?.toUpperCase() === 'CONDUCTOR').length, [unassignedCrews]);

  const categories = [
    { title: 'Drivers', count: driversCount, route: '/user/crews/pending/drivers' },
    { title: 'Conductors', count: conductorsCount, route: '/user/crews/pending/conductors' },
  ];

  return (
    <PageContainer>
      <PageHeader title="Pending" backHref="/user/crews" />
